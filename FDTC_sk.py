...
class FairDecisionTreeClassifier():
    ...
    def fit(self, X="X", y="y", s="s", **kwargs):
        ...
        check_array(X, dtype=["float", "object"])
        if "pandas" not in str(type(X)):
            X = pd.DataFrame(X)

        self._X = X
        self._y = np.array(y).astype(int)

        # for compatibility with scikit-learn since sklearn fit() methods only take X, y
        if "pandas" not in str(type(s)):
            if s == "s":
                if "fit_params" not in list(kwargs.keys()):
                    self._s = np.zeros(len(X))
                else:
                    self._s = np.array(kwargs["fit_params"]["s"]).astype(str)
            else:
                self._s =  np.array(s).astype(str)
        else:
            self._s = np.array(s).astype(str)


        if (len(self._X) != len(self._y)) or (len(self._X) != len(self._s)) or (len(self._y) != len(self._s)):
            raise ValueError("X, y, and s lenghts do not match")
        if len(self._y.shape) == 1 or len(self._y.ravel()) == len(self._X):
            self._y = self._y.ravel()
        if len(self._s.shape) == 1 or len(self._s.ravel()) == len(self._X):
        ...
        if (self.sampling_proportion != 1.0) or (self.bootstrap):
            ...
            split_groups = (
                pd.DataFrame(self._s).apply(lambda x: "_".join(x), axis=1).astype(str) + "_" + pd.Series(self._y).astype(
                    str) if (
                        self._s.shape[1] > 1
                ) else (
                        pd.Series(self._s.ravel()).astype(str) + "_" + pd.Series(self._y).astype(str)
                )
            )
            all_indexs = np.array(range(len(self._X)))
            ...
            self._X = self._X.iloc[indexs_to_keep]
            self._y = self._y[indexs_to_keep]
            self._s = self._s[indexs_to_keep]

        # computing once
        self._y_pos_bool = self._y == 1
        self._y_neg_bool = ~self._y_pos_bool
        self._s_bool_dict = {}
        for s_column in range(self._s.shape[1]):
            self._s_bool_dict[s_column] = {}
            unique_s = np.unique(self._s[:, s_column])
            for s in unique_s:
                self._s_bool_dict[s_column][s] = (self._s[:, s_column]) == s
        # for binary-unicategorical methods
        self._s_pos_bool = self._s_bool_dict[0][self._s[0, 0]]
        self._s_neg_bool = ~self._s_pos_bool
        ...
        if "int" in str(type(self.max_features)):
            self._features = sorted(np.random.choice(
                self._X.columns,
            ...
            self._features = sorted(np.random.choice(
                self._X.columns,
                size=max(1, int(round(self._X.shape[1] * self.max_features))),
                replace=False
            ))
        elif ("auto" in str(self.max_features)) or ("sqrt" in str(self.max_features)):
            self._features = sorted(np.random.choice(
                self._X.columns,
                size=max(1, int(round(np.sqrt(self._X.shape[1])))),
                replace=False
            ))
        elif "log" in str(self.max_features):
            self._features = sorted(np.random.choice(
                self._X.columns,
                size=max(1, int(round(np.log2(self._X.shape[1])))),
                replace=False
            ))
        else:
            self._features = self._X.columns
        self._X = self._X[self._features]

        numerics = ['int16', 'int32', 'int64', 'float16', 'float32', 'float64']
        categorical_part = self._X.select_dtypes(exclude=numerics)
        numeric_part = self._X.select_dtypes(include=numerics)
        # OHE to be applied to X from predict method
        self._ohe = OneHotEncoder(handle_unknown="ignore").fit(categorical_part)
        # X is now an array
        self._X = np.concatenate(
            (
                numeric_part.values.astype(float),
                self._ohe.transform(categorical_part).toarray()
            ), axis=1
        )

        # ~_feature_value_idx_bool[feature][value]: feature >= value
        self._feature_value_idx_bool = {}
        for feature in range(self._X.shape[1]):
            self._feature_value_idx_bool[feature] = {}
            # discarding the first value since there is no "less than lowest"
            unique_values = np.unique(self._X[:, feature])
            if len(unique_values) >= 2:
                ...
                for value in unique_values[1:]:
                    idx_bool = (self._X[:, feature] < value)
                    self._feature_value_idx_bool[feature][value] = idx_bool

        # prediction threshold
        self._pred_th = self._y.sum() / len(self._y)
        self._indexs = np.repeat(True, len(self._X))

        scaff_parent = (1 - self.orthogonality) * 0.5 - self.orthogonality * 0.5

        def evaluate_split(feature, value, indexs):
            left_bool = self._feature_value_idx_bool[feature][value] & indexs
            right_bool = (~self._feature_value_idx_bool[feature][value]) & indexs
            ...
            if (left_bool.sum() >= self.min_samples_leaf) and (right_bool.sum() >= self.min_samples_leaf):
                ...
                # auc_y
                tpr_y = ((self._y_pos_bool) & left_bool).sum() / ((self._y_pos_bool) & indexs).sum()
                fpr_y = ((self._y_neg_bool) & left_bool).sum() / ((self._y_neg_bool) & indexs).sum()
                ...
                auc_s_list = []
                for s_column in range(self._s.shape[1]):
                    unique_s = np.unique(self._s[indexs, s_column])
                    # if more than 1 sensitive attribute is present
                    if len(unique_s) >= 2:
                        for s in unique_s:
                            s_pos = self._s_bool_dict[s_column][s]
                            s_neg = ~s_pos
        ...
        def get_best_split(indexs):
            ...
            best_feature = np.nan
            for feature in range(self._X.shape[1]):
                unique_values = np.unique(self._X[indexs, feature])
                if len(unique_values) >= 2:
                    unique_intersect = np.intersect1d(
                        unique_values[1:],
                        np.array(list(self._feature_value_idx_bool[feature].keys()))
                    )
        ...
        def build_tree(indexs, depth=0):
            tree = {}
            if (
                    len(np.unique(self._y[indexs])) == 1 or (  # no need to split if there is already only 1 y class
                ...
                class_prob = (self._y_pos_bool & indexs).sum() / indexs.sum()
                return class_prob
                ...
                if np.isnan(feature):  ## in case no more feature values exist for splitting
                    ...
                    left_indexs = self._feature_value_idx_bool[feature][value] & indexs
                    right_indexs = (~self._feature_value_idx_bool[feature][value]) & indexs

                    tree[(feature, value)] = {
                        "prob": (self._y[indexs]).sum() / indexs.sum(),
                        "<": build_tree(left_indexs, depth=depth + 1),
                        ">=": build_tree(right_indexs, depth=depth + 1),
                    }
                    return tree

        self._tree = build_tree(self._indexs)
        self._is_fit = True
        return self
    ...
    def predict_proba(self, X, theta=None):
        ...
        y_prob = get_prob(X, self._tree).reshape(-1, 1)
    ...
    def _more_tags(self):
        return {"binary_only": True,
                "requires_y": True,
                "X_types": ["2darray", "string", "categorical"]}
...
class FairRandomForestClassifier():
    ...
    def fit(self, X="X", y="y", s="s", **kwargs):
        ...
        self._trees = trees
        self.classes_ = np.unique(y)
        self._pred_th = (y == 1).sum() / len(y)
        self._s = np.array(s).astype(object) if (
        ...
        if self.n_jobs == 1:
            ...
            batches_trees = make_batches(self._trees, n_jobs=self.n_jobs)
            ...
            self._trees = [tree for fit_batch_trees in fit_batches_trees for tree in fit_batch_trees]
    ...
    def predict_proba(self, X, theta=None, mean_type="prob"):
        ...
        if self.n_jobs == 1:
            if mean_type == "prob":
                y_prob = np.mean(
                    [tree.predict_proba(X, theta=theta)[:, 1] for tree in self._trees],
                    axis=0
                ).reshape(-1, 1)

            elif mean_type == "pred":
                y_prob = np.mean(
                    [tree.predict(X, theta=theta) for tree in self._trees],
                    axis=0
                ).reshape(-1, 1)
    ...
    def predict(self, X, theta=None, mean_type="prob"):
        ...
        mean_type -> str
            {"prob", "pred"}
            Method to compute the probailities across all trees, with which the np.mean([0.5, self._pred_th]) is the threshold
            Note: self._pred_th is given as the proportion of positive class instances [P(Y=1)]
            "prob" computes the mean of all tree probabilities (the probability of Y=1 of each terminal node)
            "pred" computes the mean of all tree predicitons {0, 1}
        """
        return (self.predict_proba(X, theta, mean_type)[:, 1] >= np.mean([0.5, self._pred_th])).astype(int)
