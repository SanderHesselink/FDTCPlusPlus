...
class FairDecisionTreeClassifier():
    ...
    def fit(self, X="X", y="y", s="s", **kwargs):
        ...
        for s_col in range(self.s.shape[1]):
            self.s[:,s_col] = pd.util.hash_array(vals=self.s[:,s_col])
        # computing once
        ...
        categorical_part = self.X.select_dtypes(exclude=numerics).to_numpy()
        ...
        # Because features end up ordered, we can infer whether or not a feature is
        # categorical by comparing its index to the amount of numerical values
        self.categorical = len(numeric_part.columns)
        ...
        def get_best_split(indexs):
            ...
            for feature in range(self.X.shape[1]):
                ...
                if len(unique_values) >= 2:
                    # np.intersect1d converts the numbers to float and causes one million issues
                    # I don't like this solution either, but it's the best I've got
                    unique_intersect = np.intersect1d(
                        list(unique_values),
                        list(self.feature_value_idx_bool[feature].keys())
                    )
                    #unique_intersect =[]
                    #for item in unique_values:
                    #    if item in list(self.feature_value_idx_bool[feature].keys()):
                    #        unique_intersect.append(item)
